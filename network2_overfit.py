...
dense2 = tf.layers.dense(inputs=dense1, units=4096, activation=tf.sigmoid)
...
with tf.Session() as sess:
    ...
    labels_ = round_01(labels).astype(np.float32)
    ...
    for step in range(TRAIN_STEPS):
        ...
        if step % 5 == 0:
            ...
            train_accuracy = accuracy.eval(feed_dict=
                {input_object: features["objects"], 
                input_scene: features["scenes"], 
                ground_truth:labels_, mode_is_train: False})
